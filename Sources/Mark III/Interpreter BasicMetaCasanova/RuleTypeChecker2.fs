  ...
  exc{
    ...
    | RuleNormalizer2.Conditional(l,c,r) ->
      ...
      if type_left = type_right then
        let res = (Conditional{left = left; predicate = c; right = right})
...
let type_check_rule (ctxt:RuleCheckerCtxt) (next:NormalizedRule)
  ...
  exc{
    ...
    let prem = List.map (fun x -> (x,0)) premise.Premise
    let res = {side_effect = false ; input = input ; output = output ;
               premis = List.rev prem ; typemap = premise.TypeMap;
               definition = Position.Zero}
    return (name,res)
