...
let prioritize (l:List<'a>) ariety priority merge =
  ...
  | [x] -> [x]
  | x::xs ->
    let l_by_priority = l |> List.mapi (fun i x -> x,i) |> List.sortBy priority |> List.rev 
    ...
    for x,i in l_by_priority do
      ...
      let x_node = l_nodes_by_element.[x]
    ...
    let res = [ for x in l_nodes do if x.Collapsed |> not then yield x.Element ]
    res