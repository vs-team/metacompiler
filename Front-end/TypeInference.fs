...
type TypeAnnotation = 
  | Unknown // ground, primitive, predefined keywords
...
let rec addNullTypeAnnotations =
  function
  | Keyword(k,pos,()) -> Keyword(k,pos,Unknown)
  | Application(b,args,pos,()) -> Application(b,[ for a in args -> addNullTypeAnnotations a ],pos,Unknown)
  | Extension(e,pos,()) -> Extension(e,pos,Unknown)
  | Imported(i,pos,()) -> Imported(i,pos,Unknown)
...
let inferTypeAnnotations (input:BasicExpression<Keyword, Var, Literal, Position, Unit>)
  ...
  addNullTypeAnnotations input,
  addNullTypeAnnotations output,
  [ for c in clauses -> addNullTypeAnnotations c ]
// (*
