module ParserMonad
#nowarn "40"

open Utilities

do System.Threading.Thread.CurrentThread.CurrentCulture <- System.Globalization.CultureInfo.GetCultureInfo("EN-US")

type Parser<'a> = { Parse : List<char> -> List<'a * List<char>> }
  with
    static member Make(p:List<char> -> List<'a * List<char>>) : Parser<'a> = { Parse = p }
    static member (+) (p1:Parser<'a>, p2:Parser<'b>) : Parser<Either<'a,'b>> = 
     (fun buf ->
        match p1.Parse buf with
        | [] ->
          match p2.Parse buf with
          | [] -> []
          | p2res ->
            [ for res,restBuf in p2res -> Second res, restBuf ]
        | p1res ->
          [ for res,restBuf in p1res -> First res, restBuf ]) |> Parser.Make
    static member (!!) (p:Parser<'a>) : Parser<'a> = 
      (fun buf -> 
        [
          for x,res_buf in p.Parse buf do
          yield x, buf
        ]) |> Parser.Make
and ParserBuilder() =
  member this.Bind(p:Parser<'a>, k:'a->Parser<'b>) : Parser<'b> =
   (fun buf ->
      [
        let all_res = p.Parse buf
        for res,restBuf in all_res do
        yield! (k res).Parse restBuf
      ]) |> Parser.Make
  member this.Combine(p:Parser<'a>, k:Parser<'b>) : Parser<'b> =
   (fun buf ->
      [
        let all_res = p.Parse buf
        for res,restBuf in all_res do
        yield! k.Parse restBuf
      ]) |> Parser.Make
  member this.Return(x:'a) : Parser<'a> =
    (fun buf -> [x,buf]) |> Parser.Make
  member this.ReturnFrom(x:Parser<'a>) : Parser<'a> = x
  member this.Yield(x:'a) : Parser<'a> = this.Return x
  member this.Zero() = this.Return()
  member this.Delay(f:unit -> Parser<'a>) = f()
  member this.Run(f:Parser<'a>) = f


let parser = ParserBuilder()
let p = parser  

let fail() = 
  (fun _ -> []) |> Parser.Make 

let character(c:char) : Parser<char> = 
 (fun buf ->
  match buf with
  | x::cs when x = c -> [c, cs]
  | _ -> []) |> Parser.Make

let word (s:string) =
  let rec word (w:List<char>) : Parser<List<char>> =
    p{
      match w with
      | x::xs ->
        let! c = character x
        let! cs = word xs
        return c::cs
      | [] -> 
        return []
    }
  word(s |> Seq.toList)

let rec takeWhile (s:Parser<'a>) : Parser<List<'a>> =
  p{
    let something = 
      p{ let! c = s
         let! cs = takeWhile s
         return c::cs }
    let nothing = p { return [] }
    let! res = something + nothing
    match res with
    | First l -> return l
    | Second l -> return l
  }

let rec character' (p:char -> bool) : Parser<char> =
 (function 
  | c::cs when p c -> [c, cs]
  | buf -> []) |> Parser.Make

let rec takeWhile' (p:char -> bool) : Parser<List<char>> =
 (function 
  | c::cs when p c ->
    [ for res,resBuf in (takeWhile' p).Parse cs -> c::res,resBuf ]
  | buf -> [[],buf]) |> Parser.Make

let isDigit c = c >= '0' && c <= '9'
let isAlpha c = (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z')

let intLiteral = 
  p{
    let! c = character' isDigit
    let! cs = takeWhile (character' isDigit)
    let s = new System.String(c::cs |> Seq.toArray)
    return s |> System.Int32.Parse
  }

let floatLiteral = 
  p{
    let! i = intLiteral
    let! dot = character '.'
    let! c = character' isDigit
    let! cs = takeWhile (character' isDigit)
    let s = new System.String('0' :: '.' :: c::cs |> Seq.toArray)
    let f = s |> System.Double.Parse
    return float i + f
  }  

let identifier =
  p{
    let! c = character' (fun c -> isAlpha c || c = '_')
    let! cs = takeWhile (character' (fun c -> isAlpha c || isDigit c || c = '-' || c = '_' || c = '\'' ))
    return new System.String(c::cs |> Seq.toArray)
  }

let stringLiteral =
  p{
    let! q1 = character '\"'
    let! s = identifier
    let! q2 = character '\"'
    return s
  }

let tab = 
  p{
    let! t = (character '\t') + (word "  ")
    return ()
  }

let eof = 
  (function
   | [] -> [(),[]]
   | _ -> []) |> Parser.Make
 
let newline = word "\r\n" + word "\n\r" + character '\n'

let blank_space = takeWhile (character ' ')

let empty_line = 
  p{
    let! bs = blank_space
    let! nl = newline
    return ()
  }

let rec empty_lines = 
  p{
    let! el = empty_line
    let! els = empty_lines + p.Zero()
    return ()
  }
