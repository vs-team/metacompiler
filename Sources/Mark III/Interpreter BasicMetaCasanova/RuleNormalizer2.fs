...
let normalize_right_premistree (right_prem:PremisFunctionTree)
  ...
  prs{
    ...
    | ParserTypes.Literal(lit,pos) -> 
      return! fail (ParserError (sprintf "right of premis may not have literals%A" pos))
    | ParserTypes.RuleBranch(x) ->
      return! fail (ParserError (sprintf "right of premis may not have a rule%A" x.Pos))
...
let normalize_rules 
  :Parser<string*ParseScope,string
