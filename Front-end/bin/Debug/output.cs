...
class _opMultiplication : IRunnable
{
  ...
  public IEnumerable<IRunnable> Run()
  {
    ...
    }
    {
      ...
      if (tmp_1 != null)
      {
        var a = tmp_1.P1; var b = tmp_0.P2;
        foreach (var tmp_2 in (a).Run())
        {
          var a_Prime = tmp_2;
          foreach (var tmp_3 in (b).Run())
          {
            var b_Prime = tmp_3;
            foreach (var tmp_4 in (new _opMultiplication(a_Prime, b_Prime)).Run())
            {
              var c = tmp_4;
              foreach (var tmp_5 in (c).Run())
              {
                var c_Prime = tmp_5;
                foreach (var tmp_6 in (new _opAddition(c_Prime, b_Prime)).Run())
                {
                  var d = tmp_6;
                  yield return d;
                }
    ...
    }
    {
      ...
      if (tmp_1 != null)
      {
        ...
        foreach (var tmp_2 in (a).Run())
        {
          ...
          {
            var b_Prime = tmp_3;
            foreach (var tmp_4 in (new _opAddition(a_Prime, b_Prime)).Run())
            {
              var c = tmp_4;
